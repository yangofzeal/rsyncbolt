...
BUILD='PUBLIC-LOOPBACK-LARGE-4G-V5-PORTABLE-RSYNCBOLT'
...
def find_rsyncbolt():
    ...
    for d in os.environ.get('PATH','').split(os.pathsep):
        if not d: continue
        p=os.path.join(os.path.abspath(os.path.expanduser(d)),'rsyncbolt')
        if os.path.isfile(p) and os.access(p,os.X_OK): return p
    raise RuntimeError('rsyncbolt not found: expected executable ./rsyncbolt or rsyncbolt on PATH')
...
def write_state(src,dst,helper,size,mtime_ns):
    ...
    st={'transport':{'os':'Darwin' if sys.platform=='darwin' else 'Linux','home':os.path.expanduser('~'),'remote':helper,'root':dst,'trailing':False,'sibling':'rsyncbolt','bootstrap_kind':'python-v1'},'files':{'large.bin':{'size':int(size),'mtime_ns':int(mtime_ns)}}}
    t=p+'.tmp'
...
def free_gate(exe,version):
    ...
    print('RSYNCBOLT_PUBLIC_LARGE_FREE_GATE'); print('VERSION       %s'%version); print('EXECUTABLE    %s'%exe); print('SIZE_MIB      %d'%SIZE_MIB); print('RETURN_CODE   %d'%rc); print('BUY_URL_SEEN  %s'%('YES' if url else 'NO')); print('LIMIT_MESSAGE %s'%('YES' if limit else 'NO')); print('PASS          %s'%('YES' if ok else 'NO')); print('--- rsyncbolt output ---'); print(combined.rstrip()); return 0 if ok else 1
...
def paid_speed(exe,version):
    ...
    st=os.stat(src); mt=int(getattr(st,'st_mtime_ns',int(st.st_mtime*1e9))); write_state(src,bolt+'/',exe,n,mt)
