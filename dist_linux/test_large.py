...
def write_state(src,dst,exe,size,mtime_ns):
    p=state_path(src,dst); st={'transport':{'os':'Darwin' if sys.platform=='darwin' else 'Linux','home':os.path.expanduser('~'),'remote':exe,'root':dst,'trailing':False},'files':{'large.bin':{'size':int(size),'mtime_ns':int(mtime_ns)}}}
    t=p+'.tmp'
