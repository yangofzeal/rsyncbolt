...
HOST='loopback'
BASE='/tmp/rsyncbolt_public_2g_v3'
def clock():
...
def write_state(src,dst,exe,size,mtime_ns):
    ...
    st={'transport':{'os':'Darwin' if sys.platform=='darwin' else 'Linux','home':os.path.expanduser('~'),'remote':exe,'root':dst,'trailing':False},'files':{'small.bin':{'size':int(size),'mtime_ns':int(mtime_ns)}}}
    t=p+'.tmp'
...
def main():
    ...
    st=os.stat(src); mt=int(getattr(st,'st_mtime_ns',int(st.st_mtime*1e9))); write_state(src,bolt+'/',exe,BASE_BYTES,mt)
